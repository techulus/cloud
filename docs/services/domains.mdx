...
Set `AUTO_SUBDOMAIN_DOMAIN` on the control plane to offer automatic domains in
service networking settings. For example, with
`AUTO_SUBDOMAIN_DOMAIN=apps.example.com`, a service whose private hostname is
`project-api-production` can use `project-api-production.apps.example.com`.

Create a wildcard DNS record for `*.apps.example.com` that points to all proxy
servers. The setting contains only the base domain, without `*.` or a protocol.
Custom domains remain available alongside automatic domains.
