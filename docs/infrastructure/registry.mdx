...
Techulus Cloud does not install or schedule registry garbage collection automatically.
...
Garbage collection must not race with image pushes. The following maintenance script stops the registry while collecting, so the registry is briefly unavailable. It prevents overlapping runs and restarts the registry if garbage collection fails.
...
After garbage collection, wait for the registry to report `healthy` and check its storage usage:
