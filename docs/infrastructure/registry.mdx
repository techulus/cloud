...
Images are stored on the local filesystem. The default deployment uses the
persistent Docker volume `registry-data`; fresh installs can instead configure
an [external registry data directory](/installation#separate-registry-data-volume).
Delete operations are enabled for garbage collection.
