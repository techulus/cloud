...
Registry changes are transactionally queued for every registered agent, including offline agents. Rotating a password replaces the encrypted credential while preserving the host; deleting an entry removes it from the next complete bundle. Use pull-only robot or service-account tokens wherever possible.
