...
- A user-owned Google Artifact Registry Docker repository with a repository-scoped Writer account and cleanup policy. Complete the [GAR setup](/infrastructure/registry) before installing.

You need two DNS records pointing to your server:
...
By default, the installer deploys the latest GitHub release and verifies its
Compose files against the release manifest. Set `TECHULUS_CLOUD_VERSION` before
running the script only when you need a specific release.
...
the generated token and configure `GAR_REPOSITORY` and `GAR_AGENT_KEY_BASE64`
from the [GAR setup](/infrastructure/registry).
...
<Warning>
  The GAR release is a clean beta cutover. Images and persisted references from the old bundled
  registry are not migrated. Provision GAR, add both GAR variables to `.env`, and apply its
  cleanup policy before upgrading. After the upgrade, rebuild every source-backed service.
</Warning>
...
| `GAR_REPOSITORY`       | GAR image base: `<location>-docker.pkg.dev/<gcp-project>/<repository>`               |
| `GAR_AGENT_KEY_BASE64` | Base64 JSON key for the repository-scoped Artifact Registry Writer sent to agents    |
...
| `TECHULUS_CLOUD_VERSION`      | Installed release tag. The installer defaults to the latest GitHub release.                                                               |
...
| Variable               | Description                                                               |
| ---------------------- | ------------------------------------------------------------------------- |
| `GAR_REPOSITORY`       | Required GAR repository image base, with no scheme or trailing image path |
| `GAR_AGENT_KEY_BASE64` | Required base64 Writer service-account JSON key                           |

The Writer account must be scoped to the configured repository. See
[Google Artifact Registry](/infrastructure/registry)
for provisioning, authentication, cleanup policy, and upgrade instructions.

The policy keeps the newest 10 versions per package and deletes versions in any
tag state after 30 days. An active or rollback image can disappear. Failed
builds count, and multi-platform images mean 10 versions is not 10 builds or
rollouts. Deleted services retain their newest versions. A seven-day data
restore does not guarantee that its image remains available.

After deploying Writer-only code, existing operators must replace the old
protected-tag Keep rule with the tracked policy. Then remove
`GAR_ADMIN_KEY_BASE64` from `.env` and revoke its unused admin key. Techulus
does not automate this migration.
...
Use versioned or digest-pinned image references for production updates.
