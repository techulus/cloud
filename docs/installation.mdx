...
## Separate registry data volume

On a fresh install, you can store registry data on an existing mounted filesystem
instead of a Docker-managed volume. Attach and persistently mount the volume
first. Create a directory on it, then choose **Existing mounted host directory**
when the installer asks about registry storage.

For an unattended install, add the directory to the file passed through
`--env-file`:
...
The directory must exist on a mounted filesystem separate from `/`. The mount
must have an `/etc/fstab` entry or an enabled systemd mount unit. The installer
sets the directory to mode `0700` and configures Docker to start after the backing
...
Back up the attached volume independently because your server provider may
exclude it from server snapshots and backups. If you intentionally stop using
the mount, remove
...
[registry garbage collection guide](/infrastructure/registry#garbage-collection)
to reclaim unreferenced blobs.
...
| `TECHULUS_CLOUD_REGISTRY_DATA_DIR` | Optional existing mounted directory for registry data. Set only during a fresh install. |
