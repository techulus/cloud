...
- A user-owned Google Artifact Registry Docker repository with the required service accounts and cleanup policy. Complete the [GAR setup](/infrastructure/registry) before installing.
...
the generated token and configure `GAR_REPOSITORY`, `GAR_AGENT_KEY_BASE64`, and
`GAR_ADMIN_KEY_BASE64` from the [GAR setup](/infrastructure/registry).
...
<Warning>
  ...
  registry are not migrated. Provision GAR, add all three GAR variables to `.env`, and apply its
...
| `GAR_ADMIN_KEY_BASE64` | Base64 JSON key for the repository-scoped Artifact Registry Repository Administrator retained by the control plane |
...
| `GAR_ADMIN_KEY_BASE64` | Required base64 Repository Administrator service-account JSON key         |

The two service accounts must be scoped to the configured repository. Agents
receive only the Writer credential. See [Google Artifact Registry](/infrastructure/registry)
