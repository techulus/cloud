...
AWS KMS BYOK is available for AWS-hosted dedicated control planes. Prefer deploying the control plane in your AWS account. Attach an EC2 instance profile or ECS task role to the web service. Do not store static AWS credentials in `.env`.
...
The role needs `kms:GenerateDataKey`, `kms:Encrypt`, `kms:Decrypt`, and `kms:DescribeKey` for the configured key. Restrict the key policy to the role, the exact key, and this encryption context:
