...
Manual placement requires online servers with WireGuard configured. Serverless services require proxy servers. Automatic placement is not available for stateful or volume-backed services. The legacy top-level `replicas` field remains supported when placement is unchanged. It must match the existing manual placement.
