...
5. The control plane confirms a protection tag, then a rollout deploys the image.
...
Images are built with BuildKit, assigned an immutable `revision-<revision-id>` tag, and pushed to [Google Artifact Registry](/infrastructure/registry). Each successful final image receives a `protected-revision-<revision-id>` alias before rollout. The GAR Writer credential used by BuildKit is also used by agents for Podman pulls; the control plane's Repository Administrator credential is never sent to agents.
