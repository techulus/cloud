...
4. The agent pushes the final multi-platform image to the installation's user-owned Google Artifact Registry repository.
5. A rollout deploys the image.
...
Images are built with BuildKit, assigned a unique `revision-<revision-id>` tag, and pushed to [Google Artifact Registry](/infrastructure/registry). The same repository-scoped GAR Writer credential is used for BuildKit pushes and Podman pulls. Repository cleanup policies alone control retention: keep the newest 10 versions per package and delete other versions older than 30 days, regardless of deployment status. See the registry guide for retention and rollback limitations.
...
| `pushing`   | Pushing the image to Google Artifact Registry |
