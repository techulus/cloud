...
| Agent Communication   | Pull-based HTTP          | Agent polls expected state, reports status, and long polls for queued-work wakeups |
| Source Image Registry | Google Artifact Registry | User-owned storage and provider-managed artifact cleanup                           |
...
graph TD
    ...
    GAR["User-owned Google Artifact Registry"]
    P1 -->|"HTTPS state, status, and work wait"| CP
    W1 -->|"HTTPS state, status, and work wait"| CP
    W2 -->|"HTTPS state, status, and work wait"| CP
    P1 -->|"Build push and deployment pull"| GAR
    W1 -->|"Build push and deployment pull"| GAR
    W2 -->|"Build push and deployment pull"| GAR
...
- Report status on a separate short interval and remain in `IDLE` if no drift exists.
...
set, GAR package, and generated route while reusing the normal runtime
...
runtime teardown. [GAR cleanup policies](/infrastructure/registry) manage image
retention independently of service deletion, retaining the newest 10 versions
per package and deleting other versions older than 30 days.
...
then wake on the next public request handled by that proxy. Stateless services
without volumes can use automatic placement across proxy nodes. Stateful or
volume-backed serverless services require manual placement on a proxy node.
