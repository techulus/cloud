...
- Polls the control plane every 15 seconds for expected state.
...
6. Push the image to the installation's user-owned Google Artifact Registry repository using the Writer credential supplied by the control plane.
...
When an agent is idle, it also makes a bounded long-poll request for queued-work
wakeups. The control plane uses PostgreSQL `LISTEN/NOTIFY` to end that request as
soon as work is enqueued. The notification is only a wake hint. The durable work
queue remains authoritative, and the existing status request performs the atomic
lease before the agent starts work.

The agent continues periodic status reports while long polling. If the wait
endpoint or PostgreSQL notification listener is unavailable, the agent backs off
and queued work is still discovered through the next status report.
