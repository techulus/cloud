...
import { applyManifest } from "@/lib/cli-service";
...
export async function POST(request: Request) {
	...
	try {
		const result = await applyManifest(parsed.data);
		return Response.json(result);
	} catch (error) {
		return Response.json(
			{ error: error instanceof Error ? error.message : "Failed to apply manifest" },
			{ status: 400 },
		);
	}
}
